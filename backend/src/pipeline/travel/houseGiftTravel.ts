...
export function parseIndexDate(value: string): string | null {
  ...
  // Round trip through a real date: 2/31/2025 must not become 2025-02-31,
  // which Postgres rejects at insert time, mid-run.
...
/**
 * Cheap pre-filter: the filer shares the member's last name. First names are
 * not compared, because members file under nicknames ("Tom Emmer" for
 * "Emmer, Thomas", "Mike Thompson" for "Thompson, Michael"). A staffer with the
 * member's last name passes this filter; the real member-or-staff test is the
 * Clerk's own path: member filings are served under `/gtimages/MT/`, staff
 * filings under `/gtimages/ST/`, so a staff filing never loads at the MT URL
 * the importer requires before it writes.
 */
export function isMemberOwnFiling(row: Pick<HouseGiftTravelRow, "filerName" | "memberName">): boolean {
  ...
  return last.length > 0 && normalizeName(row.filerName).includes(last);
}
  ...
  israel: {
    ...
    sponsors: [
      ...
      },
      {
        ...
        clause:
          "the J Street Education Fund, an organization tied to J Street, a group that lobbies Congress for U.S. support of Israel",
      ...
      },
      {
        ...
        followUp:
          "The organization, founded by evangelical Christian activist Heather Johnston, takes U.S. lawmakers to Israeli settlements in the West Bank and says its trips helped triple U.S. funding for Israel's Iron Dome.",
        sources: [
          "https://www.usieducation.org/tours",
          "https://www.usieducation.org/leadership",
          "https://www.foxnews.com/culture/iron-dome-abraham-accords-congressional-trips-shaped-15-years-us-israel-cooperation",
        ],
      },
      {
        key: "12tribe",
      ...
      },
      {
        ...
        clause:
          "the Jewish Community Relations Council of New York, a Jewish community group that promotes support for Israel",
        sources: [
      ...
      },
      {
        ...
        clause:
          "UJA-Federation of New York, a Jewish organization that funds Jewish causes in New York and Israel",
        sources: [
 ...
 *
 * With a legislator index the member is the matched legislator, so two
 * members of one state who share a surname and travel the same day stay two
 * trips, and one member printed two ways ("Deutch, Theodore" and "Deutch,
 * Theodore E.") stays one. A row that matches nobody falls back to state +
 * surname; the importer reports it as unresolved either way.
 */
export function groupHouseSponsoredTrips(
  ...
  for (const row of rows) {
    ...
    const match = legislators
      ? matchHouseTripLegislator({ ...row, departureDate: row.departureDate }, legislators)
      : null;
    const member =
      match?.outcome === "matched" ? `bioguide:${match.legislator.bioguide}` : `${row.state}|${memberLastName(row.memberName)}`;
    const key = `${member}|${row.departureDate}`;
...
export type ExistingTripRecord = { id: string; record_identity_key: string; retired_at: string | null };

export type SponsoredTripPlan =
  | { action: "unchanged"; recordId: string; keyIndex: number }
  | { action: "changed" | "retired_blocks"; recordId: string }
  | { action: "insert" };

/**
 * What to do with one trip, given every stored record that is this trip:
 * rows holding one of its possible identity keys, plus rows this importer
 * wrote for it earlier (found by run-id prefix, so a reworded sentence still
 * finds them). A retired row blocks the trip even when the wording has since
 * changed: a withdrawn record must not come back through a re-run.
 */
export function planSponsoredTripRecord(
  existing: readonly ExistingTripRecord[],
  candidateKeys: readonly string[]
): SponsoredTripPlan {
  const live = existing.filter((record) => !record.retired_at);
  const sameKey = live.find((record) => candidateKeys.includes(record.record_identity_key));
  if (sameKey) {
    return { action: "unchanged", recordId: sameKey.id, keyIndex: candidateKeys.indexOf(sameKey.record_identity_key) };
  }
  if (live.length > 0) {
    return { action: "changed", recordId: live[0]!.id };
  }
  const retired = existing.find((record) => record.retired_at);
  return retired ? { action: "retired_blocks", recordId: retired.id } : { action: "insert" };
}
