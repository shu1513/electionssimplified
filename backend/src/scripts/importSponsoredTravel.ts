...
import { syncRollCallRecordTags } from "../pipeline/rollcall/rollCallFanOut.js";
import {
...
async function main(): Promise<void> {
  ...
  const { trips, undated } = groupHouseSponsoredTrips(indexRows, destination);
  ...
  try {
    ...
    const labels = [{ researchAreaSlug: destination.researchAreaSlug, stance: destination.stance }];
    ...
    for (const trip of trips) {
      ...
      const existing = await pool.query<{
        ...
        `
          SELECT id, record_identity_key, retired_at::text AS retired_at, origin_run_id
      ...
      const sameKey = existing.rows.find((record) => candidateKeys.includes(record.record_identity_key));
      if (sameKey) {
        row.recordId = sameKey.id;
        row.sourceUrl = candidateUrls[candidateKeys.indexOf(sameKey.record_identity_key)]!;
        if (sameKey.retired_at) {
          row.action = "retired_blocks";
          row.detail = "a retired record holds this identity key";
          continue;
        }
        row.action = "unchanged";
        if (!dryRun) {
          await syncRollCallRecordTags(pool, sameKey.id, labels, researchAreaIdBySlug);
        }
        continue;
      }
      const earlier = existing.rows.find((record) => !record.retired_at);
      if (earlier) {
        row.action = "changed";
        row.recordId = earlier.id;
        row.detail = "this trip already has a record with a different sentence or source; left alone";
        continue;
      }
      ...
      if (!reachable) {
        ...
        row.detail = "no filing PDF for this trip returned HTTP 200";
        continue;
      }
      sourceUrl = reachable;
      ...
      try {
        ...
        await syncRollCallRecordTags(client, recordId, labels, researchAreaIdBySlug);
